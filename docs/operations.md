...
aws s3api list-object-versions \
  --bucket <BUCKET> --prefix "shared/example.xlsx" --region ap-northeast-1

# (2) 削除マーカーを削除する (--version-id は DeleteMarkers 側の VersionId)
aws s3api delete-object \
  --bucket <BUCKET> --key "shared/example.xlsx" \
  --version-id <DELETE_MARKER_VERSION_ID> --region ap-northeast-1
```
...
aws s3api get-object \
  --bucket <BUCKET> --key "shared/example.xlsx" \
  --version-id <VERSION_ID> restored.xlsx --region ap-northeast-1

# (3) 同じキーへアップロードし直す (これが新しい現行バージョンになる)
aws s3 cp restored.xlsx "s3://<BUCKET>/shared/example.xlsx" --region ap-northeast-1
```
...
デプロイします。デプロイの詳細は下記「11. Amplify Hosting のデプロイと運用」を参照してください。
...
- [ ] ビルドインスタンスを `XLARGE_72GB` から縮小する。恒久対応は AWS サポートに起票して `STANDARD_8GB` へ戻す。下記「11. Amplify Hosting のデプロイと運用」を参照
