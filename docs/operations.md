...
> sandbox と本番は**別プール**です。`amplify_outputs.json` は最後に生成した環境を指すため、
> ローカルで sandbox を動かした後はその値が sandbox プールになります。利用者 / パスキー / 認証アプリは
> プールごとに独立するので、sandbox で認証を試すときは、sandbox プールの ID に対して下記のセクション 1 (利用者の追加) の
> 手順で利用者を作り直してください。

> 本ガイドの手順には、パスワードをコマンドへ直接書くものはありません。仮パスワードは Cognito が
> 生成し、本人へメールで送付します。例外的に `admin-set-user-password` などでパスワードを引数に
> 渡す場合は、シェルの履歴に平文で残るため、作業後に `history -d <番号>` などで該当行を消して
> ください。
...
このプールはパスキー (WebAuthn) を第一要素の `WEB_AUTHN` として許可しているため、Cognito 側でパスワードレスと
みなされ、仮パスワードの自動生成が行われません。そのため作成時は `--temporary-password` で
仮パスワードを明示的に渡す必要があります。下記のように、その場で生成して変数経由で渡し、
値は画面に出さずに作業します。

```bash
# (1) 利用者を作成する、仮パスワードはその場で生成して渡し招待メールで本人へ届く
...
- 本人が招待メールの仮パスワードでサインインすると、新しいパスワードの設定を求められ、
  続けて MFA 用の認証アプリ (TOTP) のセットアップが求められます
...
パスワードを忘れた利用者は自分で再設定できます。管理者の操作は不要です。
...
- 確認コードは、サインイン画面の「パスワード」欄に入れるものでは**ありません**。「パスワードを
  お忘れですか?」から進んだ先の、コード入力欄に入れます。パスワード欄に入れると「入力が無効です」
  になります
- リセットの完了に MFA は要求されず、メールの確認コードと新しいパスワードのみで完了します。
  ただしサインインには認証アプリかパスキーが必須のため、メールが侵害されただけではアカウントは
  奪われません

## 4. 認証アプリ (TOTP) を紛失した利用者への対応

Cognito は登録済みの認証アプリを、管理者側でリセット / 削除できません。これは公式仕様で、
`admin-set-user-mfa-preference` で無効化しても TOTP シークレット本体は残り、サインイン時に引き続き
コードを要求されます。この挙動は実機で確認済みです。したがって対応は本人のサインイン可否で分かれます。

- 本人がまだサインインできる場合
  - パスキーを登録しているか、現行の認証アプリがまだ使えるなら、「アカウント設定 > 認証アプリ (TOTP) の変更 / 移行」で本人が新しい認証アプリに付け替えられます
  - 管理者操作は不要です
- 認証アプリもパスキーも失い、完全にサインインできない場合
  - 管理者が TOTP をリセットする手段はありません。利用者を削除して作り直してください
  - S3 上のファイルは失われません
  ...
  削除後、セクション 1 (利用者の追加) の手順で作り直し、必要ならグループを割り当て直します。

認証アプリは紛失時にリセットできないため、あらかじめパスキーを 1 つ登録するよう案内してください。
登録手順はセクション 5 (パスキーの管理) を参照してください。

## 5. パスキーの管理

- 利用者自身が「アカウント設定 > パスキー」で登録 / 一覧 / 削除ができます。1 人あたり 20 個まで登録できます
- 登録はこの設定画面上で完結します。別画面への遷移はなく、ボタンを押すとブラウザが本人確認を求めます
- 認証アプリは 1 人 1 つだけですが、パスキーは端末ごとに複数登録できます
- パスキー単体でサインインでき、そのサインインは MFA を満たした扱いになります。認証アプリのコード入力は不要です

## 6. 誤って削除 / 上書きしたファイルの復元

バケットはバージョニングが有効なため、削除 / 上書きから 30 日以内であれば復元できます。この
30 日は非現行バージョンの保持日数で、`amplify/app.config.ts` の
`dataNoncurrentVersionExpirationDays` で決まります。30 日を過ぎると旧バージョンは自動で失効し、
復元できません。気付いたらすぐ着手してください。復元は利用者ではなく、管理者が AWS CLI で行います。
...
デプロイします。デプロイの詳細はセクション 10 (Amplify Hosting のデプロイと運用) を参照してください。
...
- 画像の縦横比は 1:1 から 4:1 の範囲にしてください。範囲外だとデプロイ時に 400 エラーになります
- SVG ではなく PNG を使ってください。Cognito 側で SVG の一部が加工されて日本語が崩れることが
  あるためです

## 9. ログの確認方法

AWS マネジメントコンソールの CloudWatch Logs Insights で検索します。フィールドインデックスを
...
- RP ID にフロントエンドのドメインを指定する
  - `app.config.ts` の `webAuthnRelyingPartyId` に SPA のドメイン storage-browser.www.non-97.net を指定します
  - auth ドメインを指定すると SPA の子ドメイン扱いになり、SPA からの登録ができません
- AllowedFirstAuthFactors に WEB_AUTHN を含める
  - `user-pool-hardening.ts` の `signInPolicy` で第一要素に `WEB_AUTHN` を許可します
  - 無いと「WebAuthn not enabled for this pool」になります
- App Client の ExplicitAuthFlows に ALLOW_USER_AUTH を含める
  - 選択ベース認証 (USER_AUTH) フローを有効にします
  - 無いと「USER_AUTH flow is not enabled for the client」になります

`ALLOW_USER_AUTH` を有効にすると平文パスワードによるサインイン経路も許可されますが、MFA 必須
設定で守られています。パスキー単体でのサインインが MFA を満たす点はセクション 5 (パスキーの管理) を参照してください。

## 12. トラブルシューティング

| 症状 | 考えられる原因 | 対処 |
|---|---|---|
| サインイン画面が出ない / エラーになる | カスタムドメインの反映待ち (最大 1 時間) <br>`amplify_outputs.json` が古い | git push で再デプロイするか `aws amplify start-job` で再ビルドする |
| 認証アプリの登録で「入力が無効です」と出る | WAF がブロックしている可能性 | `amplify/monitoring/cognito-waf.ts` の TOTP 除外設定を確認する <br>WAF ログでブロック元を調べる |
| デプロイが UPDATE_ROLLBACK になる | 多くは設定値やリソース依存の問題 | CloudFormation コンソールでスタックイベントの失敗理由を確認する |
| 設定変更が反映されない | Amplify のビルド失敗や未完了 | git push 後のビルド結果を Amplify コンソールか `aws amplify list-jobs` で確認する |
| 画面が真っ白になる | `amplify_outputs.json` の欠落や CSP 違反が典型的 | ブラウザのコンソールでエラーを確認する |
